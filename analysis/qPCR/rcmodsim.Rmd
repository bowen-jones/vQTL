...
rc.simu <- t(matrix(replicate(10, simu.rc.normal.multi()), nrow = 13))
rc.simu <- as.data.frame(rc.simu)
names(rc.simu) <- c("beta0", "beta1", "beta2", "rbeta3", "beta4", "beta5", "gamma0", "gamma1", "gamma2", "gamma3", "gamma4", "gamma5")
...
```{r}
rcsim <- read.csv("C://Users//jones//Documents//STT//HPC output//rcsim1k.csv")
rcsim$parameters <- c(39,1,2,rep(0,3),-5,3,4,rep(0,4))
rcsim <- rcsim[,c(1,1004,1002,1003, 2:1001)]
write.csv(rcsim, "rcsim1k.csv")
```

cALCULATE MSE
```{r}
for (i in 1:n.runs){
  (rcsim[1,i+4]-rcsim[1,1])^2
}
```
