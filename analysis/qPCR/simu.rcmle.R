logL<-function(para, simu.obs.func)
{
  ...
  suppressMessages(attach(simu.obs.func));
...
simu.rc.logis<-function(n.obs=10^3, 
                        alp.val=30, beta.val=2, mu.sig=-1, gam.val=3,
                        cut.off=40)
{
...
simu.rc.logis(); ## the function will return the estimater of RC model, then the complete model later.
 
 
 
logN<-function(para, simu.obs.func)
{
  ## para[1] is the intercept of the mean model, 2 is beta, 3 os musigma, 4 is gamma
  suppressMessages(attach(simu.obs.func));
  wi<- (yi.star-para[1]-x.val*para[2])/(exp(para[3]+z.val*para[4]));
  val.loglike<-sum(delt.val*wi^2/2)+sum(delt.val*(para[3]+z.val*para[4]))-sum((1-delt.val)*log(1-pnorm(wi)+10^(-9)))
  detach(simu.obs.func);
  return(val.loglike);
}
logN.comp<-function(para, simu.obs.func)
{
  ## para[1] is the intercept of the mean model, 2 is beta, 3 os musigma, 4 is gamma
  suppressMessages(attach(simu.obs.func));
  wi<- (yi.star-para[1]-x.val*para[2])/(exp(para[3]+z.val*para[4]));
  val.loglike<-sum(wi^2/2)+sum((para[3]+z.val*para[4]));
  detach(simu.obs.func);
  return(val.loglike);
}
 
simu.rc.normal<-function(n.obs=10^3, 
                         ...
                         cut.off=40)
{
  ...
  rc.mle<-optim(par=rep(0,4),logN, simu.obs.func=simu.obs)$par;
  com.mle<-optim(par=rep(0,4),logN.comp, simu.obs.func=simu.obs)$par;
