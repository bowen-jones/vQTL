...
set.seed(12345);
out.normal.multi<-simu.rc.normal.multi(n.obs=10^3,beta.val.multi=c(39,1,2,rep(0,3)),fun.type='normal');
para.current<-c(25,0.5,2.5,rep(1,3),-2.9,2,3,rep(1,4));

ourbje.mle1<-optim(par=para.current,  method="BFGS",obj.func.bje,flist.data=out.normal.multi[[3]],sep=6,which.obj=0,control=list(maxit=10^5))
#para.current<-ourbje.mle1$par;
print(ourbje.mle1$par)

## @knitr BJE.try1

## @knitr optim.try1
set.seed(12345);
#fun.type %in% c('normal','logis','runif','cauchy'))
...
## @knitr optim.try2
para.current<-c(rep(1,13))
ourbje.mle2<-optim(par=para.current,  method="BFGS",obj.func.bje,flist.data=out.normal.multi[[3]],sep=6,control=list(maxit=10^5))
print(ourbje.mle2)

para.current<-c(25,0.5,2.5,rep(1,3),-2.9,2,3,rep(1,4));
ourbje.mle3<-nlminb(start=para.current,obj.func.bje,flist.data=out.normal.multi[[3]],sep=5+1,control=list(eval.max=10^5,iter.max=10^5))

print(ourbje.mle3)
## @knitr nlminb.try2
para.current<-c(rep(1,13))
ourbje.mle4<-nlminb(start=para.current,obj.func.bje,flist.data=out.normal.multi[[3]],sep=5+1,control=list(eval.max=10^5,iter.max=10^5))
